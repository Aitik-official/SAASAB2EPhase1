...
export default function Header({ showNav = true }: { showNav?: boolean }) {
  const router = useRouter();
    ...
    useEffect(() => {
        ...
        if (isNotificationsModalOpen || isProfileModalOpen || isMobileMenuOpen) {
    ...
    }, [isNotificationsModalOpen, isProfileModalOpen, isMobileMenuOpen]);
    ...
    return (
        <header className="bg-transparent px-4 sm:px-6 py-5">
            <div className="mx-auto flex max-w-7xl items-center justify-between">
                ...
                <nav
                    className="relative hidden lg:flex items-center gap-1 px-2 py-2.5 rounded-full"
                    style={{
                        background: '#FFFFFF',
                ...
                >
                    ...
                    {indicatorStyle && navItems.some(item => isActive(item.path)) && (
                        <div
                            className={`absolute bg-slate-800 rounded-full ${isInitial ? '' : 'transition-all duration-500 ease-in-out'}`}
                            style={{
