...
async function uploadWorkExperienceDocuments(req, res) {
  try {
    ...
    const uploadsDir = path.join(__dirname, '../../uploads/work-experience');
    ...
    for (const file of files) {
      const fileName = `${candidateId}_${Date.now()}_${file.originalname}`;
      const filePath = path.join(uploadsDir, fileName);
      
      // Save file to disk
      fs.writeFileSync(filePath, file.buffer);
      
      // Store file URL (relative to /uploads)
      const fileUrl = `/uploads/work-experience/${fileName}`;
      uploadedFiles.push({
...
async function uploadAcademicAchievementDocuments(req, res) {
  try {
    ...
    // Create uploads directory if it doesn't exist
    const uploadsDir = path.join(__dirname, '../../uploads/academic-achievement');
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }
    ...
    for (const file of files) {
      ...
      // Store file URL (relative to /uploads)
      const fileUrl = `/uploads/academic-achievement/${fileName}`;
      uploadedFiles.push({
        name: file.originalname,
        url: fileUrl,
        size: file.size,
...
async function uploadInternshipDocuments(req, res) {
  try {
    ...
    // Create uploads directory if it doesn't exist
    const uploadsDir = path.join(__dirname, '../../uploads/internship');
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }
    ...
    for (const file of files) {
      ...
      // Store file URL (relative to /uploads)
      const fileUrl = `/uploads/internship/${fileName}`;
      uploadedFiles.push({
        name: file.originalname,
        url: fileUrl,
        size: file.size,
...
async function uploadLanguageDocuments(req, res) {
  try {
    ...
    // Create uploads directory if it doesn't exist
    const uploadsDir = path.join(__dirname, '../../uploads/languages');
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }
    ...
    for (const file of files) {
      ...
      const fileUrl = `/uploads/languages/${fileName}`;
...
async function uploadProjectDocuments(req, res) {
  try {
    ...
    // Create uploads directory if it doesn't exist
    const uploadsDir = path.join(__dirname, '../../uploads/project');
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }
    ...
    for (const file of files) {
      ...
      const fileUrl = `/uploads/project/${fileName}`;
      uploadedFiles.push(fileUrl);
    }
...
async function uploadCompetitiveExamDocuments(req, res) {
  try {
    ...
    // Create uploads directory if it doesn't exist
    const uploadsDir = path.join(__dirname, '../../uploads/competitive-exam');
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }
    ...
    for (const file of files) {
      ...
      const fileUrl = `/uploads/competitive-exam/${fileName}`;
      uploadedFiles.push(fileUrl);
    }
...
async function uploadAccomplishmentDocuments(req, res) {
  try {
    ...
    // Create uploads directory if it doesn't exist
    const uploadsDir = path.join(__dirname, '../../uploads/accomplishment');
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }
    ...
    for (const file of files) {
      ...
      const fileUrl = `/uploads/accomplishment/${fileName}`;
      uploadedFiles.push({
        name: file.originalname,
        url: fileUrl,
        size: file.size,
...
async function uploadCertificationDocuments(req, res) {
  try {
    ...
    // Create uploads directory if it doesn't exist
    const uploadsDir = path.join(__dirname, '../../uploads/certification');
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }
    ...
    for (const file of files) {
      ...
      const fileUrl = `/uploads/certification/${fileName}`;
      uploadedFiles.push({
        name: file.originalname,
        url: fileUrl,
        size: file.size,
...
async function uploadVisaDocuments(req, res) {
  try {
    ...
    // Create uploads directory if it doesn't exist
    const uploadsDir = path.join(__dirname, '../../uploads/visa-work-authorization');
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }
    ...
    for (const file of files) {
      ...
      const fileUrl = `/uploads/visa-work-authorization/${fileName}`;
      uploadedFiles.push(fileUrl);
    }
...
async function uploadVaccinationDocuments(req, res) {
  try {
    ...
    // Create uploads directory if it doesn't exist
    const uploadsDir = path.join(__dirname, '../../uploads/vaccination');
    if (!fs.existsSync(uploadsDir)) {
      fs.mkdirSync(uploadsDir, { recursive: true });
    }
    ...
    for (const file of files) {
      ...
      const fileUrl = `/uploads/vaccination/${fileName}`;
      uploadedFiles.push(fileUrl);
    }
...
async function uploadProfilePhoto(req, res) {
  try {
    ...
    if (existingProfile?.profilePhotoUrl) {
      const oldFilePath = path.join(__dirname, '../../uploads', existingProfile.profilePhotoUrl.replace('/uploads/', ''));
    ...
    const sanitizedFileName = `profile_${candidateId}_${timestamp}${fileExtension}`;
    ...
    // Store file URL (relative to /uploads)
    const fileUrl = `/uploads/profile-photos/${sanitizedFileName}`;
