...
async function verifyOTP(req, res) {
  try {
    ...
    let latestOTP = await prisma.otpVerification.findFirst({
      where: {
        candidateId: candidate.id,
        status: OtpStatus.PENDING,
      },
    ...
    if (!latestOTP) {
      ...
      const otpWithCandidate = await prisma.otpVerification.findFirst({
        where: {
          status: OtpStatus.PENDING,
          candidate: {
            whatsappNumber: fullWhatsAppNumber,
    ...
    if (candidate.id !== candidateId) {
      ...
      let correctCandidate = await prisma.candidate.findUnique({
        where: { id: candidateId },
      });
      ...
      if (!correctCandidate) {
        ...
        try {
          ...
          const otpCount = await prisma.otpVerification.updateMany({
            where: {
              candidateId: candidate.id,
            },
            data: {
          ...
          const verifyCandidate = await prisma.candidate.findUnique({
            where: { id: candidateId },
          });
          if (verifyCandidate) {
        ...
        correctCandidate = await prisma.candidate.update({
          where: { id: candidateId },
          data: {
            whatsappNumber: fullWhatsAppNumber,
            countryCode: countryCode,
